...
int D3::mmain(arguments args) {
    ...
    auto xx=readFile<std::string>(filename);
    A1(xx);
    A2(xx);
    return 0;
}
...
int D3::A2(std::vector<std::string> const &inp) {
    ...
    size_t oxGenRate= bin2Int(inp2[0]);
    inp2=inp;
    for(int i=0;(i<inp[0].size())&(inp2.size()>1);i++)
    {
        auto is1=[&i](std::string c)
    ...
    size_t Co2ScrubberRate= bin2Int(inp2[0]);
    std::cout<<Co2ScrubberRate*oxGenRate;
    return 0;
}
